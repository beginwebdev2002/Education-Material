...
export class GenerationFormComponent implements OnInit, OnDestroy {
  ...
  saveDraft(): void {
    if (typeof window !== 'undefined') {
      localStorage.setItem(this.DRAFT_KEY, JSON.stringify(this.model()));
      this.draftExists.set(true);
  ...
  clearDraft(): void {
    if (typeof window !== 'undefined') {
      const defaultFormats: Omit<MaterialTypes, 'id'> = { program: true, lecture: false, presentation: false, test: false };
      ...
      this.model.set({
        ...
        selectedLevel: defaultLevels.length > 0 ? defaultLevels[0] : '',
        topic: '',
