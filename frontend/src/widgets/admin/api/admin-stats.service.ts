...
import { ActivityEntry, ActivityType, AdminAnalytics, AdminOverview, AnalyticsCategory, OnlineUser } from '../model/admin-stats.model';
...
export class AdminStatsService {
    ...
    analytics(days = 30, category?: AnalyticsCategory): Observable<AdminAnalytics> {
        let params = new HttpParams().set('days', days);
        if (category) {
            params = params.set('category', category);
        }
        return this.http.get<AdminAnalytics>(ADMIN_ENDPOINTS.ANALYTICS.url, { params });
    }
